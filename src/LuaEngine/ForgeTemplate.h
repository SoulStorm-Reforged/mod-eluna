...
public:
    static int thunk(lua_State* L)
    {
        ...
        {
            FORGE_LOG_ERROR("[Forge]: {} returned unexpected amount of arguments {} out of {}. Report to devs", l->name, args, expected);
            ASSERT(false);
        }
    ...
    static void SetMethods(Forge* E, luaL_Reg* methodTable)
    {
        ASSERT(E);
        ASSERT(methodTable);
...
public:
    template<typename T>
    ForgeObject(T * obj, bool manageMemory);
    ...
    bool IsValid() const { return !callstackid || callstackid == sForge->GetCallstackId(); }
    ...
    void SetValid(bool valid)
    {
        ...
        if (valid)
            if (CanInvalidate())
                callstackid = sForge->GetCallstackId();
            else
                callstackid = 0;
        else
            callstackid = 1;
...
public:
    ...
    static void Register(Forge* E, const char* name, bool gc = false)
    {
        ASSERT(E);
        ASSERT(name);
    ...
    template<typename C>
    static void SetMethods(Forge* E, ForgeRegister<C>* methodTable)
    {
        ASSERT(E);
        ASSERT(tname);
        ASSERT(methodTable);
    ...
    static int Push(lua_State* L, T const* obj)
    {
        ...
        // Create new userdata
        ForgeObject** ptrHold = static_cast<ForgeObject**>(lua_newuserdata(L, sizeof(ForgeObject*)));
        if (!ptrHold)
        {
            FORGE_LOG_ERROR("{} could not create new userdata", tname);
            lua_pushnil(L);
            return 1;
        }
        *ptrHold = new ForgeObject(const_cast<T*>(obj), manageMemory);
        ...
        if (!lua_istable(L, -1))
        {
            FORGE_LOG_ERROR("{} missing metatable", tname);
            lua_pop(L, 2);
            lua_pushnil(L);
            return 1;
        }
    ...
    {
        ForgeObject* forgeObj = Forge::CHECKTYPE(L, narg, tname, error);
        if (!forgeObj)
            return NULL;
        ...
        if (!forgeObj->IsValid())
        {
            ...
            {
                FORGE_LOG_ERROR("{}", buff);
            }
            return NULL;
        }
        return static_cast<T*>(forgeObj->GetObj());
    }
    ...
    {
        ForgeObject* forgeObj = Forge::CHECKOBJ<ForgeObject>(L, 1);
        bool invalidate = Forge::CHECKVAL<bool>(L, 2);

        forgeObj->SetValidation(invalidate);
        return 0;
    }

    static int CallMethod(lua_State* L)
    {
        T* obj = Forge::CHECKOBJ<T>(L, 1); // get self
        if (!obj)
            return 0;
    ...
    {
        T* obj = Forge::CHECKOBJ<T>(L, 1, true); // get self
    ...
    static int Equal(lua_State* L) { Forge::Push(L, Forge::CHECKOBJ<T>(L, 1) == Forge::CHECKOBJ<T>(L, 2)); return 1; }
...
template<typename T>
ForgeObject::ForgeObject(T * obj, bool manageMemory) : callstackid(1), _invalidate(!manageMemory), object(obj), type_name(ForgeTemplate<T>::tname)
{
    SetValid(true);
}

template<typename T> const char* ForgeTemplate<T>::tname = NULL;
template<typename T> bool ForgeTemplate<T>::manageMemory = false;
